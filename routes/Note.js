...
router.post('/note/add', async (req,res,next) => {
    ...
    result ? res.status(201).json({message: "Note added", note: result })
...
// Working
//Get all notes
router.get("/note/all/:user_id", async (req,res,next) => {
try {
    let {user_id} = req.params;

    let notes = await Notes.find({user_id}).sort({_id: -1}).exec()
